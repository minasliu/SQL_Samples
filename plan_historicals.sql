/*****************************************************************************/
/*     File Name     : plan_historicals.sql                                    */
...
SPOOL /home/minal/HardDrives/Plan/TotalBooks.csv
...
SELECT	c.kc_desc_descr kc_desc, 
	a.cbd_fiscal_year_qtr fq,
	a.cbd_week_of_fiscal_qtr fw,
	'    ' qtr_wk,
	SUM(b.book_keycomp_qty) book_qty
FROM	cust_bk_date_cd_v a,  
	cust_book_keycomp_v b,
	keycomp_desc_item_cd_v c, 
	mkt_item_cd_v m
WHERE 	a.cbd_date_pk = b.cbd_date_pk
AND   	b.kc_desc_item_pk = c.kc_desc_item_pk
AND   	b.mkt_item_pk = m.mkt_item_pk
AND   	(a.cbd_fiscal_year_qtr =
		(SELECT fiscal_year_qtr
    		FROM   date_cd_v
    		WHERE  TO_CHAR(calendar_date,'MM/DD/YYYY') = TO_CHAR(sysdate,'MM/DD/YYYY'))
OR	a.cbd_fiscal_year_qtr =
		(SELECT fiscal_year_qtr
    		FROM   date_cd_v
    		WHERE  TO_CHAR(calendar_date,'MM/DD/YYYY') = TO_CHAR(sysdate-91,'MM/DD/YYYY')))
...
GROUP BY c.kc_desc_descr, 
	a.cbd_fiscal_year_qtr,
	a.cbd_week_of_fiscal_qtr
...
SPOOL /home/minal/HardDrives/Plan/OtherBooks.csv
CREATE TABLE raw_data AS
SELECT	c.kc_desc_descr kc_desc, 
	a.cbd_fiscal_year_qtr fq,
	a.cbd_week_of_fiscal_qtr fw,
...
AND	platform_name='OTHER'
GROUP BY kc_desc
ORDER BY kc_desc;

SPOOL OFF;

SPOOL /home/minal/HardDrives/Plan/TotalFill.csv
PROMPT Total Fill To Date
select to_char(sysdate,'MM-DD-YYYY HH:MI:SS')|| ',,,,,' from dual;
PROMPT kc_desc,fill,
SELECT 	kc_desc || ',' ||
	sum(fill) || ','
